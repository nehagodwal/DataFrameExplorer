...
class DataFrameTableView(QTableWidget):
	...
	# Hide default indices in favor of extra column added to headers.
        ...
        for i in range(len(self.model.getDataFrame().index)):
            for j in range(len(self.model.getDataFrame().columns)):
                ...
                if isnull(self.model.getDataFrame().iat(i,j)):
    ...
    def addCol(self, cn):
        """Adds an empty col to the dataframe and view.

        Args:

        Returns:
            Empty Column with name input by user
        """
        colName = cn
        ...
        #Add to dataframe
        emptydata = ''
        df[colName] = Series(data=emptydata, index=df.index, dtype=None)
        ...
        for i in range(self.rowCount()):
            self.setItem(i, colCnt, MyTableWidgetItem(df[colName][i], self))

    def duplicateCol(self, cn):
        """Adds an duplicate col to the dataframe and view based on the column name specified by user .
        ...
        colName = cn
        newColName = 'CopyOf_'+colName

        colCnt = self.columnCount()
        colnum = int(colCnt)
        copycolNum = 0

        #Insert empty column in a view
        self.insertColumn(colCnt)

        #insert empty column in a dataframe
        emptydata = ''
        df[newColName] = Series(data=emptydata, index=df.index, dtype=None)

        #set column name
        headers = [""] + list(self.model.getDataFrame().columns.values)
        self.setHorizontalHeaderLabels(headers)

        # get the index of input column name
        l = -2
        ...
        #copy values of input column in new inserted column in dataframe
        for j in range(self.rowCount()):
            df.iat[j, colnum-1] = df.iat[j, copycolNum]


        # Add copied column to view.
        for i in range(self.rowCount()):
            self.setItem(i, colCnt, MyTableWidgetItem(df[newColName][i], self))

    def deleteCol(self, col):
        """Deletes a col based on name.

        Args:
            col: name of col to be deleted.
        """
        ...
        headers = [""] + list(self.model.getDataFrame().columns.values)
        # Find the correct col index to be removed from view
        # get the index of input column name
        l = -2
        for cname in headers:
            l += 1
        ...
        print "delete col name: ", deletecolNum

        try:
            self.removeColumn(deletecolNum+1)
        except UnboundLocalError:
            success = False
            error = QMessageBox.about(self, 'Error', 'Column does not exist!')

        # If succesfully removed from view, remove from dataframe
        if (success):
            try:
                column_name = str(col)
                self.model.getDataFrame().drop(column_name, axis=1, inplace=True)
    ...
    def duplicateRow(self, row):
        ...
        for i in range(self.rowCount()-1):
            viewIndex = int(self.item(i, 0).text())
            rownum = int(row)
            if viewIndex == rownum:
                savedValue = viewIndex
                break;
        ...
        # Create row to be added.
        for i in range(len(df.dtypes)):
            emptyList.append(df.loc[savedValue][i])
...
class DataFrameDistView(QListWidget):
    """ A view containing a list of distinct values by column
        to be displayed in a window.
    ...
    def __init__(self, model, col, parent=None):
        """Create the list and add items to it for each column.
        ...
        """
        self.colName = col
        super(DataFrameDistView, self).__init__(parent)
        self.model = model
        
        currentFrame = self.model.getDataFrame()
        
        listDistinct = currentFrame[self.colName].unique()
        for i in listDistinct:
            self.addItem(i)
