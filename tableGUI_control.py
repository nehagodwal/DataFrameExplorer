...
class EditCellDialog(QDialog):  # @UndefinedVariable
    ...
    def accepted(self):
        ...
        #print "yash: ", str(dataframe.iloc[rowIndex,columnIndex-1])
...
class ReplaceAllDialog(QDialog):
    """Prompts user for searching string and sting to be replaced with.
    ...
    def __init__(self, view):
        ...
        # Create the form.
        self.findkw = QLineEdit(self)
        self.replacewith = QLineEdit(self)

        self.buttonBox = QDialogButtonBox(QDialogButtonBox.Ok | QDialogButtonBox.Cancel)
        self.form = QFormLayout(self)

        # Add labels.
        labelFindkw = QLabel('Find All:', self)
        labelReplacewith = QLabel('Replace With:', self)
        
        self.form.addRow(labelFindkw, self.findkw)
        self.form.addRow(labelReplacewith, self.replacewith)
        self.form.addWidget(self.buttonBox)
    ...
    def accepted(self):
        """Take user's input when Ok is pressed. Will do error checking on the string. Then, updates view and model.
        ...
        # try to convert to bool, int, floats, or object based on value
        # change corresponding model
        find = self.findkw.text()
        replace = self.replacewith.text()
        columnIndex= -1
        rowIndex = -1

        view = self.view   
        dataframe = view.model.getDataFrame()

        #find string to be replaced in each row and each column.
        for i in range(len(dataframe)): # for each row

            for j in range(len(dataframe.columns)): # for each column
                if str(find) in str(dataframe.iloc[i,j]):
                    string = str(dataframe.iloc[i,j])
                    string1 = string.replace(find, replace)
                    rowIndex = i
                    columnIndex = j
                    ...
                    for k in range(len(dataframe)):
                        ...
                        if(viewIndex==rowIndex):
                            self.view.setItem(k, columnIndex+1, MyTableWidgetItem(string1, self.view))
                            break;
                    #replace in dataframe
                    dataframe.set_value(dataframe.index[rowIndex],dataframe.columns[columnIndex],str(string1))

        item = view.item(2, 0)

        if columnIndex == -1 or rowIndex == -1:
            error = QMessageBox.about(self, 'Error', 'STRING NOT FOUND')
...
class DataFrameController(QDialog):
    ...
    def __init__(self, idNum, manager, view, parent=None):
        ...
        if (isinstance(self.view, DataFrameStatView) or isinstance(self.view, DataFrameDistView)):
        ...
        if not (isinstance(self.view, DataFrameStatView) or isinstance(self.view, DataFrameHistogramView) or isinstance(self.view, DataFrameDistView)):
        ...
        #### Version-2 changes start
        saveMenu = self.menu.addMenu('Save')
        sampleMenu = self.menu.addMenu('Take Sample')
        #### Version-2 changes End
                
    # Specify menu actions with methods.
        #### Version-2 changes start
        numberSample = QAction('Sample by number', self)
        numberSample.triggered.connect(self.prepareSampleByNum)
        sampleMenu.addAction(numberSample)

        fracSample = QAction('Sample by fraction', self)
        fracSample.triggered.connect(self.prepareSampleByFrac)
        sampleMenu.addAction(fracSample)
        
        saveFile = QAction('Save to CSV', self)
        saveFile.triggered.connect(self.prepareSaveFile)
        ...
        #### Version-2 changes start
        duplicateRow = QAction('Duplicate Row', self)
        duplicateRow.triggered.connect(self.prepareDuplicateRow)
        editMenu.addAction(duplicateRow)
        #### Version-2 changes End
        ...
        addCol.triggered.connect(self.prepareAddCol)
        editMenu.addAction(addCol)

        duplicateCol = QAction('Duplicate Column', self)
        duplicateCol.triggered.connect(self.prepareDuplicateCol)
        editMenu.addAction(duplicateCol)

        deleteCol = QAction('Delete Column', self)
        deleteCol.triggered.connect(self.prepareDeleteCol)
        editMenu.addAction(deleteCol)

        replaceAll = QAction('Replace All', self)
        replaceAll.triggered.connect(self.prepareReplaceAll)
        editMenu.addAction(replaceAll)
        #### Version-2 changes End
        ...
        #### Version-2 changes start
        distinctVal = QAction('Distinct Values', self)
        distinctVal.triggered.connect(self.prepareDistWindow)
        statMenu.addAction(distinctVal)
        #### Version-2 changes End
    ...
    #### Version-2 changes start
    def prepareDuplicateRow(self):
        """Connect to the table's duplicateRow() method and duplicate specified row.
        ...
        text, ok = QInputDialog.getText(self, 'Input Dialog', 'Duplicate row:')
        if ok:
            # If valid, verify formatting and duplicate the row.
            self.view.duplicateRow(str(text))

    def prepareAddCol(self):
        """Connect to the table's addCol() method and add new col.
        ...
        # Obtain col name from user.
        text, ok = QInputDialog.getText(self, 'Input Dialog', 'Column Name:')
        colname = str(text)
        if ok:
            self.view.addCol(colname)


    def prepareDuplicateCol(self):
        """Connect to the table's duplicateCol() method and duplicate specified column.
        ...
        for i in range(self.view.columnCount()):
            if (self.view.horizontalHeaderItem(i).text() == colname and colname != '' ):
                self.view.duplicateCol(colname)
                columnExists = True
                break;
        if not columnExists:
            error = QMessageBox.about(self, 'Error', 'Column does not exist!')

    def prepareDeleteCol(self):
        """Connect to the table's deletecol() method and delete specified row.
        ...
        # Obtain col name from user.
        text, ok = QInputDialog.getText(self, 'Input Dialog', 'Delete col:')
        ...
        for i in range(self.view.columnCount()):
            if (self.view.horizontalHeaderItem(i).text() == colname and colname != ''):
                self.view.deleteCol(colname)
                columnExists = True
                break;
        if not columnExists:
            error = QMessageBox.about(self, 'Error', 'Column does not exist!')    # If valid, verify formatting and delete the col.
    ...
    def prepareSaveFile(self):
        ...
        if ok:
            view = self.view
            df = view.model.getDataFrame()
            df.to_csv(str(text))

    def prepareSampleByNum(self):
        """Take sample by number from user
        """
        # Obtain number of records user wants to sample.
        text, ok = QInputDialog.getText(self, 'Input Dialog', 'Enter Number:')

        try:
            number = int(text)
        except ValueError:
            error = QMessageBox.about(self, 'INVALID NUMBER',
                                      'Input number should be: \n   1) INTEGER \n   2) Less than equal to "Total number of rows" AND Greater Than 0')

        rowCnt = self.view.rowCount()
        if (number > rowCnt) or (number <= 0):
            error = QMessageBox.about(self, 'INVALID NUMBER',
                                      'Input number should be: \n   1) INTEGER \n   2) Less than equal to "Total number of rows" AND Greater Than 0')
        elif ok:
            view = self.view   
            ...
            #Take sample from dataframe without replace and display in separate window
            df1 = df.sample(n=int(text), replace=False)
        self.manager.createTableWindow(df1, "Sample by number: " + text, self.view)

    def prepareSampleByFrac(self):
        """Take sample by fraction from user
        """
        # Obtain file name from user.
        text, ok = QInputDialog.getText(self, 'Input Dialog', 'Enter Fraction:')

        try:
            fraction = float(text)
        except ValueError:
            error = QMessageBox.about(self, 'INVALID FRACTION',
                                      'Input number should be: \n   1) Fraction \n   2) Less than equal to 1 AND Greater Than 0')
        ...
        if fraction > 1 or fraction <= 0:
            ...
            #print "input: ", text
            # Take sample from dataframe without replace and display in separate window
            df1 = df.sample(frac=float(text), replace=False)
        self.manager.createTableWindow(df1, "Sample by fraction: " + text, self.view)
            
    def prepareReplaceAll(self):
        """Connect to the table's ReplaceAllDialog class and replace all the occurrences of the input string.
        ...
        # Create the dialog window and display it to user.
        window = ReplaceAllDialog(self.view)
        window.show()
        window.exec_()
    ...
    def prepareDistWindow(self):
        ...
        for i in range(self.view.columnCount()):
            if (self.view.horizontalHeaderItem(i).text() == colname and colname != ''):
                df = self.view.model.getDataFrame()
                listDistinct = df[colname].unique()
                #print listDistinct
                columnExists = True
                break;
        if not columnExists:
        ...
        self.manager.createDistWindow(self.view, colname)
...
#### Version-2 changes start
    def createDistWindow(self, view, colname):
        """ Creates a new window displaying distinct values based on input column name.

        Args:
            view: Take the view's model and display its distinct values by column.
            
        Returns:
        """
        cn = colname
        view = DataFrameDistView(view.model, cn)
        controller = DataFrameController(self.generateId(), self, view)
        ...
        controller.setWindowTitle("Statistics: Distinct Values in column " "'"+ cn+"'")
        controller.show()
